...
working.directory <- paste(
  ...
  "../../Results/Simulated dataset 4 - study 1 - constrained space - new truths/normal_con",
  sep="/"
)
...
draw.sd <- 0.05                     # (Float) Standard deviation of distribution from which true se and sp values are drawn
prior.sd <- c(0.05, 0.15)           # (Float) Standard deviation of priors for se and sp in JAGS model

# Set true values for tests 2:5
se <- c(0.33, 0.68, 0.44, 0.43)
sp <- c(0.94, 0.56, 0.80, 0.87)

# True values for pi, se1, sp1
pi.true <- seq(0.05, 0.45, 0.1)
se.1 <- seq(0.05, 0.95, 0.1)
sp.1 <- seq(0.55, 0.95, 0.1)
...
pi.true.limit <- c(0, 0.5)            # (Integers) Lower and upper limit for prevelance values
se.true.limit <- c(0, 1)                 # (Integers) Lower and upper limit for sensitivity values
sp.true.limit <- c(0.5, 1)               # (Integers) Lower and upper limit for specificity values
...
run <- function(data){
  ...
  sim.results <- array(
    data=NA,
    dim=c(length(prior.sd), length(n.badgers), length(n.tests), 4, 2 * max(n.tests) + 1),
    dimnames=list(
      prior.sd=prior.sd,
      n.badgers=n.badgers,
      n.tests=n.tests, 
  ...
  for (p in 1:length(prior.sd)){ 
    ...
    for (b in 1:length(n.badgers)){
      for (t in 1:length(n.tests)){
        ...
        #saveRDS(output, paste("output_", data$sim, "_", prior.sd[p], "_", n.badgers[b], "_", n.tests[t], "_", Sys.Date(), ".RData", sep=""))
        saveRDS(c(), paste("output_", data$sim, "_", prior.sd[p], "_", n.badgers[b], "_", n.tests[t], "_", Sys.Date(), ".RData", sep=""))
        # Store some of the outputs
        sim.results[p, b, t, "true", "pi"] <- values$true[data$sim,"pi"]
        sim.results[p, b, t, "true", get.names(n.tests[t])$se] <- values$true[data$sim, names$se]
        sim.results[p, b, t, "true", get.names(n.tests[t])$sp] <- values$true[data$sim, names$sp]
        sim.results[p, b, t, "mean", "pi"] <- output$mean$pi
        sim.results[p, b, t, "mean", get.names(n.tests[t])$se] <- output$mean$se
        sim.results[p, b, t, "mean", get.names(n.tests[t])$sp] <- output$mean$sp
        sim.results[p, b, t, "sd","pi"] <- output$sd$pi
        sim.results[p, b, t, "sd", get.names(n.tests[t])$se] <- output$sd$se
        sim.results[p, b, t, "sd", get.names(n.tests[t])$sp] <- output$sd$sp
        sim.results[p, b, t, "error","pi"] <- sim.results[p, b, t, "true", "pi"] - sim.results[p, b, t, "mean", "pi"]
        sim.results[p, b, t, "error", get.names(n.tests[t])$se] <- sim.results[p, b, t, "true", get.names(n.tests[t])$se] - sim.results[p, b, t, "mean", get.names(n.tests[t])$se]
        sim.results[p, b, t, "error", get.names(n.tests[t])$sp] <- sim.results[p, b, t, "true", get.names(n.tests[t])$sp] - sim.results[p, b, t, "mean", get.names(n.tests[t])$sp]
      }
    }
  }
  return (sim.results)
...
results <- array(
  data=NA,
  dim=c(n.sim, length(prior.sd), length(n.badgers), length(n.tests), 4, 2 * max(n.tests) + 1),
  dimnames=list(
    sim=1:n.sim, 
